...
/// 与引擎无关（无 Unity API 依赖），dotnet 可独立跑通冒烟测试。
/// </summary>
public class Board
{
    public int Size { get; }
    ...
    /// <summary>按 GTP 协议记录的着法列表，例如 ["B D4", "W Q16", ...]</summary>
    ...
    public static string ToGtpVertex(int x, int y, int size)
    {
        if (x < 0 || x >= size || y < 0 || y >= size)
            throw new ArgumentOutOfRangeException($"坐标 ({x},{y}) 超出 {size}x{size} 棋盘");
    ...
    public static (int x, int y) FromGtpVertex(string v, int size)
    {
        ...
        while (i < v.Length && char.IsLetter(v[i]))
        {
            ...
            if (c == 'I')
                throw new FormatException($"GTP 坐标中不应出现字母 I: {v}");
            int colNum = c - 'A' + 1;
            if (c > 'I') colNum--;
            x = x * 25 + colNum;
            i++;
        ...
        if (x < 0 || x >= size || y < 0 || y >= size)
            throw new FormatException($"坐标 {v} 转换后 ({x},{y}) 超出 {size}x{size} 棋盘");
        return (x, y);
    }
