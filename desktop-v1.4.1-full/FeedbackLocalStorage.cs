...
public static class FeedbackLocalStorage
{
    ...
    public static void OpenMailClientWithPrefilled(string htmlPath)
    {
        ...
        // 主题：固定前缀，方便开发者筛选
        string subject = "ChinaGo 玩家反馈";

        // 正文：固定提示语，提醒玩家附件是哪个文件
        // 用 Uri.EscapeDataString 转义空格/换行/中文
        string body =
            "你好，这是一条 ChinaGo 玩家反馈。\n\n" +
            "📎 附件说明：\n" +
            "请把刚才弹出的文件夹里的这个 HTML 文件拖到本邮件里当附件发送 →\n" +
            System.IO.Path.GetFileName(htmlPath) + "\n\n" +
            "（双击 HTML 用浏览器打开，能看到完整反馈：分类/标题/详情/联系方式/对局/KataGo 日志/系统环境）\n\n" +
            "感谢支持！🙏";
