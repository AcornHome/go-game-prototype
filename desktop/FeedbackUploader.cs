        ...
        string? userId)
    {
        ...
        try
        {
            ...
            var payload = new
            {
                ...
                category = string.IsNullOrWhiteSpace(record.Category) ? "Other / suggestion" : record.Category,
                title = record.Title,
        ...
        string lastErr = "Cannot reach the feedback server";
    ...
    private static async Task<Result> PostOnceAsync(string baseUrl, string json)
    {
        try
        {
            ...
            if (!resp.IsSuccessStatusCode)
                return Result.Fail($"Server returned {(int)resp.StatusCode}");
            ...
            return Result.Fail(string.IsNullOrWhiteSpace(err) ? "The server rejected this feedback" : err!);
        }
        catch (TaskCanceledException)
        {
            return Result.Fail("Connection timed out (the server may be down)");
        }
        catch (HttpRequestException)
        {
            return Result.Fail("Cannot connect to the server (it may be down)");
        }
        catch (Exception ex)
        {
            return Result.Fail(ex.Message);
