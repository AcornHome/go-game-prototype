...
#pragma warning restore CS0067
    ...
    public KataGoClient(string exePath, string modelPath, string configPath, int boardSize)
    {
        if (!File.Exists(exePath))
            throw new FileNotFoundException($"KataGo executable not found: {exePath}\nDownload: https://github.com/lightvector/KataGo/releases");
        if (!File.Exists(modelPath))
            throw new FileNotFoundException($"KataGo model weights not found: {modelPath}");
        if (!File.Exists(configPath))
            throw new FileNotFoundException($"KataGo config file not found: {configPath}");
        ...
        EnqueueLog($"Initializing {boardSize}×{boardSize} board (first launch compiles the neural net, please wait)...");
        SendCommand($"boardsize {boardSize}");
        EnqueueLog($"KataGo ready ({boardSize}×{boardSize} board).");
    }
    ...
    private async Task ReaderLoop(StreamReader stdout, CancellationToken ct)
    {
        ...
        {
            EnqueueLog($"[reader] exited: {ex.Message}");
        }
    }
    ...
    {
        if (!IsRunning) throw new InvalidOperationException("KataGo process has exited");
    ...
    public string GenMove(string color)
    {
        ...
        if (resp.StartsWith("?"))
            throw new Exception($"KataGo genmove failed: {resp}");
    ...
    public string SetBoardSize(int boardSize)
    {
        if (boardSize != 9 && boardSize != 13 && boardSize != 19)
            throw new ArgumentOutOfRangeException(nameof(boardSize), $"Board must be 9 / 13 / 19, got {boardSize}");
    ...
    public (string vertex, AnalyzeResult? analysis) GenMoveAnalyzeWithResult(string color, double seconds)
    {
        ...
        if (resp.StartsWith("?"))
        {
            ...
            EnqueueLog($"[warn] kata-genmove_analyze unsupported, falling back to genmove: {resp.Split('\n')[0].Trim()}");
    ...
    public HintResult? AnalyzeHint(string color, int maxVisits, double maxSeconds, int maxCandidates = 3)
    {
        ...
        try
        {
            ...
            if (resp.StartsWith("?"))
            {
                EnqueueLog($"[warn] Hint unavailable (kata-genmove_analyze rejected): {resp.Split('\n')[0].Trim()}");
                return null;
            }
    ...
    private HintResult ParseAndUndoHint(string resp, string color, int maxCandidates)
    {
            ...
            && IsPlausibleVertex(actualMove))
        {
            ...
            if (undoResp.StartsWith("?"))
            {
                ...
                EnqueueLog("[warn] Hint trial move undo failed; will rebuild KataGo board");
            }
        }
